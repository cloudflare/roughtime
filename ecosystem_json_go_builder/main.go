...
func main() {
	...
	for _, server := range servers {
		fmt.Fprintf(file, "\tconfig.Server{\n")
		...
		for _, b := range server.PublicKey {
			fmt.Fprintf(file, "%v, ", b)
		}
		fmt.Fprintln(file, "},")

		fmt.Fprintln(file, "\t\tAddresses:     []config.ServerAddress{")
		for _, address := range server.Addresses {
			fmt.Fprintln(file, "\t\t\tconfig.ServerAddress{")
...
const (
	filePrefix string = `// Code generated by "go run ./ecosystem_json_go_builder". DO NOT EDIT.
